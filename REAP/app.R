    ...
    mainPanel(
        ...
        tabsetPanel(type = "tabs",
                    ...
                    tabPanel("Introduction", 
                             ...
                             p("The median-effect equation has been widely used for estimating 
                           dose-response, but it’s sensitive to data with extreme values. 
                           Starting from this concern,we introduce Beta regression for 
                           estimating marginal dose-response curves, which is robust even 
                           when data has extreme values."),
                             ...
                             tags$ul(
                                 ...
                                 tags$li("The columns in the input dataset should follow the order of Concentration, Effect and Agent")
                             ),
                    ...
                    tabPanel("Dataset", value = 2, 
                             fluidRow(
                                 column(12, span(textOutput("text_warning"), style="color:red")),
                                 br(),
                                 column(12, tableOutput("table"))
                             )),
                    tabPanel("Dose-response curves", value = 3,
                             plotOutput("plot123"),
...
server <- function(input, output) {
    ...
    output$text_warning <- renderText({
        if (any(dt()[,2] <=0)==TRUE | any(dt()[,2]>=1)==TRUE){
            "Observations about the data are unreliable. Some of your drug effects are out of (0,1).
